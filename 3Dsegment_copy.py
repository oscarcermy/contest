...
def calculate_face_mask(input_face, mesh, cams_list, masks, classes, depths, center):
    """
    ...
    for idx, cam in enumerate(cams_list):
        ...
        for vertex in face_vertices:
            ...
            """ if world_to_cam[2] <= 0:                      # 在相机背后
               continue """
            d_face = world_to_cam[2]
            ...
            if not (0 <= u < W and 0 <= v < H):
                continue
            d_render = depths[idx][v, u]

            scores = mask[:, v, u]
            if not any(scores):
                continue
            
            if abs(d_render + d_face) > max(ABS_EPS, REL_EPS * d_render):
                continue
            
            scores_idx = np.argmax(mask[:, v, u])
...
def main():
    ...
    masks, classes = load_masks_from_npz(mask_files, (960, 1280))

    depth = [f"D:/learing____record/contest/renders/depth_"+"0"*(3-len(str(i)))+ str(i)+".npy" for i in range(36)]
    depths = [np.load(f) for f in depth]
    ...
    for i in range(40000):
        ...
        labels.append(calculate_face_mask(input_face, mesh, cams_list, masks, classes, depths, center))
